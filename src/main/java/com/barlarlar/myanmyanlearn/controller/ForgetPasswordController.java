...
public class ForgetPasswordController {
    ...
    public String handleForgetPassword(@RequestParam("email") String email,
            Model model) {
        log.info("Forget password requested");

        boolean exists = memberRepository.existsByEmail(email);

        if (!exists) {
            log.warn("Forget password email not found");
        ...
        try {
            ...
            if (memberOpt.isEmpty()) {
                log.warn("Member missing for existing email");
                model.addAttribute("error", "Email not found. Please check and try again.");
                model.addAttribute("email", email);
                return "forget-password";
            }
            ...
            log.error("Error handling forget password", e);
